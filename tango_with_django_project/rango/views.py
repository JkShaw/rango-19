...
def index(request):
    ...
    # Dictiomary for mostviewed pages.
...
def show_category(request, category_name_slug):
    ...
    try:
        ...
        # So the .get() method returns one model instance or raises a exception
        category = Category.objects.get(slug=category_name_slug)

        # Retrieve all of the associated pages.
        # Note that filter() wil return a list of page objects or an empty list
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        if query:
            ...
            result_list = run_query(query)
    return render(
        request, 'rango/search.html', {'result_list': result_list})
    ...
    if request.method == 'GET':
        ...
        if catid:
            ...
            p = Page.objects.get_or_create(
                category=category,
                title=title, url=url)
            print('New page created: {}'.format(p))
            pages = Page.objects.filter(category=category).order_by('-views')
            # Adds our results list to the template context under name pages.
            context_dict['pages'] = pages
